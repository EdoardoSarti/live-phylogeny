...
class GNJmat:
    ...
    def genDict(self):#this dictionary will store the NJ nodes as the keys and the values as their children who can be in GNJ matrix and their distance fro this NJ node
        ...
        for i in range(len(self.labelsNJ)):
            d[self.labelsNJ[i]]=[[self.labelsNJ[i]+"1",0,i]]  
        return d
    ...
    def giveQ(self, q1,q2): 
        ...
        #GNJ part
        # multiplier= np.ones(np.shape(self.GNJpart))
        adder=np.zeros(np.shape(self.GNJpart))
        for i in range(r):
            ...
            for k in self.nodeDict[nameOfNJnode]:  # finding the children of the node
                ...
                adder[i][j]=inf   #assigning that point the value of inf so tha on addition those elements become 0
        ...
        # Qgnj=np.subtract(Qgnj,self.distsums[:,None]/(r-2))
        
        Qgnj=self.GNJpart.copy()*2*(r-2)
        Qgnj=np.subtract(Qgnj,2*self.distsums[:,None])
       
        # Qbnj=np.subtract(Qbnj,self.distsums/4)
        # Qbnj=np.subtract(Qbnj,np.diag(np.diag(Qbnj)))
    ...
    def update(self,imin,jmin,bORg,its): 
        ...
        if(bORg): #assuming true means the bnj
            ...
            r= np.shape(D0)[0]
            dfu= (self.dminBnj/(2*(r-2)) + 2*self.distsums[imin])/(2*(r-2))
            dgu= (self.dminBnj/(2*(r-2))+ 2*self.distsums[jmin])/(2*(r-2))
    ...
    # :/
            ...
            print(self.labelsNJ[imin],self.labelsGNJ[jmin])
            print("update GNJ")
            ...
            dfg= max(((gsum)/(r-1)-dfparent,dfparent)) #stores distance between the f and g nodes
    ...
    def reduceTree(self):
        ...
        for n in tr.traverse():
            if(abs(n.dist)<= 1e-10):
                if(n.up==None):
                    continue
                if(len(n.up.name)>1 and n.up.name[:3]=="Int"):
                    print(tr.get_ascii(attributes=["name","dist"],show_internal=True))
                    print(n.up.name+ " gets replaced with " + n.name)
                    n.up.name=n.name
