...
def GeneralizedNJ(d,labels):
    ...
    file1= open('../data/progress.txt','w')
    while(np.shape(matobj.DistMat)[0]>2):
        ...
        if(matobj.dminGnj<matobj.qminBnj or abs(matobj.dminGnj- matobj.qminBnj)<= 1e-7 ):
    ...
    # matobj.tr.write(format=1,outfile="/user/ragupta/home/time-aware-neighbor-joining/data/final_tree.nw")
...
# print(GeneralizedNJ(treeGen.dm,treeGen.labels).get_ascii(attributes=['name','dist'],show_internal=True))
